...
impl DataFrameWrapper {
    ...
    pub(crate) fn inner(&self) -> &datafusion::prelude::DataFrame {
        &self.inner
    }

    pub(crate) fn clone_inner(&self) -> datafusion::prelude::DataFrame {
        self.inner.clone()
    }

    pub(crate) fn set_inner(&mut self, inner: datafusion::prelude::DataFrame) {
        self.inner = inner;
    }
}
